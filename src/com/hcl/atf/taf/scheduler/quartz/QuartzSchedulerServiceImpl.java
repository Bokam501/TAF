...
import com.hcl.atf.taf.scheduler.SchedulerService;
 ...
 */
@SuppressWarnings("unchecked") public class QuartzSchedulerServiceImpl implements SchedulerService {
	...
	private static final Log log = LogFactory.getLog(QuartzSchedulerServiceImpl.class);
     ...
     *             -
     */
    public void createJob(final String productName, final String jobName, final String jobDescription,
            final String jobClassName) throws ATFException {
        ...
        JobDetail jobDetailObj = newJob(jobClass)
        							.withIdentity(jobName, productName)
     ...
     *             -
     */
    public void updateJob(final String productName, final String jobName, final String jobDescription,
            final String jobClassName) throws ATFException {
     ...
     *             -
     */
    public Date scheduleCronJob(TestConfigScheduleEntity job) throws ATFException {
        
    	Date returnValue = null;;
        if (isJobExist(job.getTestConfigName(), job.getProductName()))
        	return returnValue;
        if (job.getScheduleCronExpression() == null || job.getScheduleCronExpression().trim().equalsIgnoreCase(""))
        	return returnValue;
        
        try {
            final Class jobClass = Class.forName(job.getJobClassName());
            JobDetail jobDetail = newJob(jobClass)
            	.withIdentity(job.getTestConfigName(), job.getProductName())
            	.withDescription(job.getTestConfigDescription())
            	.storeDurably()
            	.build();
            
            final CronTrigger cronTrigger =
                    constructCronTrigger(job.getProductName(), job.getTestConfigName(), job.getTestConfigDescription(),
                            job.getScheduleCronExpression(),job.getStartDate(),job.getEndDate());
            
            log.info("Creating Cron Trigger for : " +  job.getProductName() + " : Test Run Plan : " + job.getTestConfigName() + " : Cron : " + job.getScheduleCronExpression());
     ...
     *             -
     */
    public Date reScheduleCronJob(final String productName, final String triggerOldName, final String triggerNewName,
        ...
        try {
            ...
            final CronTrigger cronTrigger = constructCronTrigger(productName, triggerName, description, cronExpString,null,null);
            
            returnValue = scheduler.rescheduleJob(new TriggerKey(triggerOldName, productName), cronTrigger);
     ...
     *             -
     */
    public Boolean deleteJob(final String productName, final String jobName) throws ATFException {
        
        boolean returnValue = false;
        
        try {
            log.info("Deleting Cron Trigger for : " +  productName + " : Test Run Plan : " + jobName);
        	if (scheduler.checkExists(new JobKey(jobName, productName))) {
            	log.info("Job Exists for deleting : " + jobName);
                returnValue = scheduler.deleteJob(new JobKey(jobName, productName));
     ...
     *             -
     */
    public void runJobNow(final String jobName, final String groupName) throws ATFException {
        try {
     ...
     *             -
     */
    public JobTriggerDetail getJobByName(final String productName, final String jobName) throws ATFException {
        ...
        try {
            ...
            final JobDetail jobDetailObj = scheduler.getJobDetail(new JobKey(jobName, productName));
            final CronTrigger trigger = (CronTrigger)getTriggerByJobName(jobName, productName);
    ...
    public JobTriggerDetail modifyJobTrigger(final JobTriggerDetail jobTriggerDetail) throws ATFException {
        ...
        updateJob(jobTriggerDetail.getJobGroup(), jobTriggerDetail.getJobName(), jobTriggerDetail.getJobDescription(),
                jobTriggerDetail.getJobClassName());
    ...
    public Map<String, Map<String, List<Date>>> getUpcomingFireTimes(List<String> productNames, Date fromDate, Date toDate) throws ATFException{
    	...
    	if (productNames == null || productNames.isEmpty()) {
    		try {
    			productNames = scheduler.getTriggerGroupNames();
    	...
    	for (String productName : productNames) {
    		...
    		try {
    			GroupMatcher gm = GroupMatcher.triggerGroupEquals(productName);
    		...
    		upcomingJobs.put(productName, upcomingTriggerTimes);
    	}
    	return upcomingJobs;
    }
    ...
    public Map<String, Map<String, List<Date>>> getUpcomingFireTimes(List<String> productNames, int numberTimes) throws ATFException{
    	...
    	for (String productName : productNames) {
    		...
    		Map<String, List<Date>> upcomingTriggerTimes = new HashMap<String, List<Date>>();
    		try {
    			log.info("Getting fire times for : " + productName);
    			GroupMatcher gm = GroupMatcher.triggerGroupEquals(productName);
    			Set<TriggerKey> triggerKeys = scheduler.getTriggerKeys(gm);
    			if (triggerKeys == null || triggerKeys.isEmpty())
    				continue;
    			for (TriggerKey triggerKey : triggerKeys) {
    				...
    				Trigger trigger = scheduler.getTrigger(triggerKey);
        			log.info("Getting fire times for Test Run Plan / Trigger key : " + triggerKey);
    				//TODO : Will null work as a calendar input ?
     ...
     *            the datasource to set
     */
    private void addNewJobForTestCycleRun(int i) {
    	
    	final String productName = "CableVision-Optimum-Mobile-App";
        final String testConfigName = "Weekly-Test-Run-Android-" + i;
        final String decription = "This job runs test cycle on android devices once a week";
        final String jobClassName = "com.hcl.atf.taf.scheduler.jobs.TestCycleRunJob";
        final String triggerName = "Weekly-Test-Run-Android-Trigger" + i;
        final String cronExpression = "0 0/1 0-1 12 9 ?";
            
        final TestConfigScheduleEntity job = new TestConfigScheduleEntity();
        job.setProductName(productName);
        job.setTestConfigName(testConfigName);
        job.setTestConfigDescription(decription);
        job.setJobClassName(jobClassName);
        job.setScheduleCronExpression(cronExpression);
    ...
    public static void main(String[] args) {
    	...
    	//Unit testing
    	QuartzSchedulerServiceImpl schedulerService = new QuartzSchedulerServiceImpl();
    	schedulerService.init();
    	schedulerService.addNewJobForTestCycleRun(3);
    }
