...
@Service
public class PMSmartScheduleDataExtractor {

	private static final Log log = LogFactory.getLog(PMSmartScheduleDataExtractor.class);

	@Autowired
	CollectionValuesScheduleMapper collectionValuesScheduleMapper;
	
	public List<ActivityCollection> readPMSamrtExcel(LinkedHashMap<String, String> dataTemplate, ArrayList<String> headerCellValueList, Sheet sheet, int totalColumnsCount, DataExtractorNotification dataExtractorNotification) {
		List<ActivityCollection> activityCollections = new ArrayList<ActivityCollection>();
		try{
			log.info("Inside readPMSamrtExcel()");
			...
			for (int rowCounter = contentRowNumber; rowCounter < totalRowsCount; rowCounter++) {
				try{
					...
					if(contentRow != null){
						...
						for (int columnCounter = 0; columnCounter < headerCellValueList.size(); columnCounter++) {
							...
							if(dataTemplate.containsValue(headerCellValueList.get(columnCounter))){
								if(cellValue != null && !cellValue.trim().isEmpty() && !"NA".equalsIgnoreCase(cellValue)){
									if((dataTemplate.containsKey("activityComponent") && dataTemplate.get("activityComponent").equalsIgnoreCase(headerCellValueList.get(columnCounter))) || (dataTemplate.containsKey("activityName") && dataTemplate.get("activityName").equalsIgnoreCase(headerCellValueList.get(columnCounter)))){
						...
						if(activityCollection.getProductName() != null && !activityCollection.getProductName().isEmpty()){
