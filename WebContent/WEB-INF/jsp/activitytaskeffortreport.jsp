				...
				<div class="col-md-12">
					    ...
					    <div class="portlet-title">
							<div class="caption caption-md">
								<i class="icon-bar-chart theme-font hide"></i>
 								<span class="caption-subject theme-font bold uppercase">Activity Task Effort Report</span>&nbsp;<span id="headerTitle" class="caption-subject theme-font"></span>
						...
						<div class="portlet-body">
							...
							<div class="col-md-4">
								<div class="input-group" id="defaultrange">								
									<input type="text" class="form-control" id="fromTodatepicker" readonly style="background-color:white"/>
									<span class="input-group-btn">
									<button class="btn default date-range-toggle" type="button" style="height:34px"><i class="fa fa-calendar"></i></button>
									</span>
								</div>
							</div> -->
							<button id="generateReport" class="btn green-haze" onclick="generateActivityTaskEffortReport()">Generate Report</button>
						<!-- 	<button id="generateExcel" class="btn green-haze" onclick="javascript:void();">Generate Excel Report</button> -->
					...
					<!-- ContainerBox -->
				 	<div id="reportTable">
						...
						<table id="activityTaskEffortTable"   class="display" cellspacing="0" width="100%">
							<thead>
							<tr></tr>
							</thead>
						</table>					
...
var urlToGetActivityStatusReportExcel="";
...
$("#activityTaskEffortTable tfoot input" ).on( 'keyup change', function () {
    that
    	.search( this.value, true, false )
        .draw();
} );
function generateActivityTaskEffortReportExcel(){
	...
	urlToGetActivityStatusReport = 'report.activity.task.effort.excel?productId='+productSelectedId;
	openLoaderIcon(); 
	$.ajax(
	{
	    type: "POST",
	    url : urlToGetActivityStatusReportExcel,
	    cache:false,
...
function generateActivityTaskEffortReport(){
	try{
		if ($('#activityTaskEffortTable').length>0) {
			$("#reportTable").children(":last").remove();
			$('#activityTaskEffortTable').remove();
			var htmlTable = '<table id="activityTaskEffortTable"><thead><tr></tr></thead></table>';
	...
	productId = (typeof productId == 'undefined') ? -1 : productId;
	productSelectedId=productId;
	urlToGetActivityStatusReport = 'report.activity.task.effort?productId='+productSelectedId;
		    ...
		    success: function(data) {
				...
				var data1=eval(data);
		  		...
		  		if(columnData == undefined){
			   	 ...
			   	 $('#activityTaskEffortTable').dataTable({			   		
			   		paging: true,			
					destroy: true,
					bJQueryUI: false,
					"scrollX":true,
...
function removeElement() {
	$.each($('#activityTaskEffortTable > tfoot>tr>th'), function(index, item){
...
function statusReportDate() {
    ...
    document.getElementById("fromTodatepicker").value = (lastmonth+' '+(currentdate-1)+','+' '+year+' '+'-'+' '+currentmonth+' '+currentdate+','+year);
    	$("#hdnDateSet").val("yes");
    	/* generateActivityTaskEffortReport(); */
  }
