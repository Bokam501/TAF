...
(function ($) {
    ...
    $.fn.extend({
        cronGen: function () {
            //create top menu
            var cronContainer = $("<div/>", { id: "CronContainer", style: "display:none;width:100%;height:100%;" });
            var mainDiv = $("<div/>", { id: "CronGenMainDiv", style: "width:100%;height:100%;" });
            var topMenu = $("<ul/>", { "class": "nav nav-tabs", id: "CronGenTabs" });
            $('<li/>', { 'class': 'active' }).html($('<a id="MinutesTab" href="#Minutes" style="padding: 10px;">Minutes</a>')).appendTo(topMenu);
            $('<li/>').html($('<a id="HourlyTab" href="#Hourly" style="padding: 10px;">Hourly</a>')).appendTo(topMenu);
            $('<li/>').html($('<a id="DailyTab" href="#Daily" style="padding: 10px;">Daily</a>')).appendTo(topMenu);
            $('<li/>').html($('<a id="WeeklyTab" href="#Weekly" style="padding: 10px;">Weekly</a>')).appendTo(topMenu);
            $('<li/>').html($('<a id="MonthlyTab" href="#Monthly" style="padding: 10px;">Monthly</a>')).appendTo(topMenu);
            $('<li/>').html($('<a id="YearlyTab" href="#Yearly" style="padding: 10px;">Yearly</a>')).appendTo(topMenu);
            ...
            $g.css({"width":"80%"});
            // Add an input
            var $i = $("<input>", { type: 'text', placeholder: 'Cron trigger'}).addClass("form-control").val($(that).val());
            $i.appendTo($g);
            // Add the button
            // -----  modified -----            
            //var $b = $("<button class=\"btn btn-default\"><i class=\"icon-edit\"></i></button>");            
            var $b = $("<button class=\"btn default\"><i class=\"fa fa-edit\" title=\"Generate Cron Expression\"></i></button>");
            ...
            /*$b.popover({
                ...
                template: '<div class="popover" style="max-width:500px !important; width:500px"><div class="popover-inner"><h3 class="popover-title"></h3><div class="popover-content"><p></p></div></div></div>',
                placement: 'bottom'
	            ...
	            // -----  modified -----
            	var $cornInputField = $("#schedulingID").find(".form-control").eq(0);
	            $b.on('click', function (e) {
	            	// -- modified --
	            	/*if($cornInputField.is('[readonly]')){ 
		            	$cornInputField.removeAttr("readOnly");
	            	}else{
	            		$cornInputField.attr("readOnly","readOnly");
	            	}*/
	            	generate(); // Generate CronExp 
	            });
	            
	            $("#schedulingID").find(".popover").remove();
	            
            	var $tabs = '<div class="popover" style="max-width: 100%;position: relative;display: table;border-collapse: separate; margin-left: 0px;margin-top: 10px;"><div class="popover-inner"><div class="popover-content" style="min-width: 455px;'
            	    +'min-height: 270px;><p>';            	
            	$tabs += $(cronContainer).html();
            	$tabs += '</p></div></div></div>';           	
            	inputElement.after($tabs);
