...
Editor.prototype._ajax = function ( data, success, error )
{
	var opts = {
		...
		success:  function ( json, status, xhr ) {
			...
			// ----- modified -----
			if(json.Result == "INFORMATION"){
				success( json );
				return;
				
			}else if(json.Result == "OK" && json.Message != null && json.Message != ""){
				callAlert(json.Message);
				success(json);
				
			}else{			
				if ( json.Result == "ERROR" || json.Record == null || json.Records == null) {
...
Editor.prototype._submit = function ( successCallback, errorCallback, formatdata, hide )
{
	...
	if ( action === "create" || action === "edit" ) {
		$.each( editFields, function ( idSrc, edit ) {
			...
			if ( ! $.isEmptyObject( changedRowData ) ) {
				...
				// ----- modified -----
				for(var keyIndex in changedRowData){
					if(keyIndex != "isActive" && keyIndex != "status" && edit.displayFields != undefined){
						var fieldKey = edit.displayFields[keyIndex];
						changedData[ idSrc ]["modifiedFieldTitle"] = fieldKey.dom.label.text();
						break;
					}
				}
			}
		...
		// ----- modified ----- 
		// for sending ajax data like jtable form data ----
		
		for(var keyIndex in allData){			
			if (allData.hasOwnProperty(keyIndex)){		
		    	for(var key in allData[keyIndex]){			
				  if (allData[keyIndex].hasOwnProperty(key))
					  
					  // -- for checkbox selection ----
					  /*if(typeof(allData[keyIndex][key]) == "object" && (allData[keyIndex][key]).length>0){
						  allData[keyIndex][key] = allData[keyIndex][key][0];
					  }else if(typeof(allData[keyIndex][key]) == "object" && (allData[keyIndex][key]).length==0){
						  allData[keyIndex][key] = 0;
					  }*/
					  
					  submitParams[key] = allData[keyIndex][key];    		  
				}
			}
		}
		
    	// for adding modified field in the form data ---- 
    	if(action === 'edit'){
    		var fieldId='';
    		var modifiedFieldID='';
    		
    		for(var fieldID in this.s.editFields){	
    			fieldId = fieldID;   			
    			var changedDataArray = [];
    			if(changedData[fieldId] != undefined){
    				changedDataArray = Object.keys(changedData[fieldId]).map(function (key) { return key; });
    			
	    			for(var modifiedID in changedData[fieldId]){
	    				if(modifiedID == "modifiedFieldTitle"){
	    					submitParams[modifiedID] = changedData[fieldId][modifiedID];    
	    				}else{
		    				modifiedFieldID = modifiedID;    		
		    				
		    				if(modifiedFieldID!=""){
		
		    				var editData = this.s.editFields[fieldId].data;
		    				for(var editiField in editData){	
		    					//if (editiField != modifiedFieldID){
		    					var editFieldIndexValu = changedDataArray.indexOf(editiField);
		    					if(editFieldIndexValu == -1){
		    						submitParams[editiField] = editData[editiField]
		    					}else{
		    						submitParams[editiField] = changedData[fieldId][editiField];
		    					}
		    				}
		    					submitParams["oldFieldValue"] = this.s.editData[modifiedFieldID][fieldId];
		    					submitParams["modifiedFieldValue"] = changedData[fieldId][modifiedFieldID];
		    				}else{
		    					submitParams["oldFieldValue"] = this.s.editFields[fieldId].data;
		    					submitParams["modifiedFieldValue"] = this.s.editFields[fieldId].data;
		    				}    	
	    				}
	    			}
	    			submitParams["oldFieldID"] = "";
	    			submitParams["modifiedFieldID"] = "";
	    			submitParams["modifiedField"] = modifiedFieldID;  
    			}
    		}
    		
    		for(var keyIndex in allData){    			
				for(var key in submitParams){			
					allData[keyIndex][key] = submitParams[key];    		  
				}
    		}    
    	}
    	// -------------------------------	
	...
	this._ajax(
		...
		function (json) {
			...
			// ----- modified -----
			if(json.Result == "INFORMATION" || json.Result == "ERROR"){
				json.error ="";	
			}
			
			if(action == "edit" && json.Result == "OK"){
				json.error ="";
			}			
			// ----- 
			...
			if ( json.error || json.fieldErrors.length ) {
				...
				// ----- modified ----- 
				if(json.Result == "OK" || json.Record != null || json.Records != null){
					var submitKeyIndex=0;
					if(json.Record != null){
						if(action === "create"){
							json.data = json.Record;
						}else{							
							//json.data = json.Record[0];
						...
						if(action === "create")
							...
							json.data = json.Records[0];
					}					
					
					if(action === 'edit' || action === 'create'){
						for(var keyIndex in submitParams.data){
							for(var submitKeyIndex in submitParams.data[keyIndex]){
								if(json.data.hasOwnProperty(submitKeyIndex) != undefined && json.data[submitKeyIndex] != null){
									submitParams.data[keyIndex][submitKeyIndex] = json.data[submitKeyIndex];
									submitParams[submitKeyIndex] = json.data[submitKeyIndex];
								}
							}
						}
					}
					json.data = submitParams;
				...
				if (json.Result == "INFORMATION" || json.Result == "ERROR"){
					...
					callAlert(json.Message);
					json.Records=[];
					json.Records.push(submitParams);
					json.data = submitParams;
					
					if(json.data.modifiedField != undefined || json.data.modifiedField != ""){						
						json.data[json.data.modifiedField] = json.data.oldFieldValue;				
						setData = json.data;					
